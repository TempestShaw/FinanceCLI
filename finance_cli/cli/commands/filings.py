...
from finance_cli.services.filings import (
    fetch_filings_with_display,
    list_filing_reports,
    list_filing_reports_with_display,
    list_filing_sections,
    read_filing_report,
    read_filing_section,
    read_filing_statement,
    read_filing_statement_with_display,
)
...
def _filings_recent(args: list[str]) -> FinanceCommandResult:
    ...
    kv = KVArgs(args[1:])
    data, display = fetch_filings_with_display(
        symbol=args[0],
...
def _filing_lookup(args: list[str]) -> tuple[str | None, str | None, str | None, KVArgs]:
    ...
    if not accession_no and len(args) >= 2 and "=" not in args[1] and _looks_like_accession(args[1]):
        accession_no = args[1]
...
def _filings_read(args: list[str]) -> FinanceCommandResult:
    ...
    section = kv.str("section")
    if section is None and not accession_no and not url:
        section = "business"
...
def _filings_statement(args: list[str]) -> FinanceCommandResult:
    ...
    data, display = read_filing_statement_with_display(
        symbol=symbol,
        accession_no=accession_no,
        url=url,
        form=kv.str("form", "10-K"),
...
def _filings_reports(args: list[str]) -> FinanceCommandResult:
    ...
    data, display = list_filing_reports_with_display(
        symbol=symbol,
        accession_no=accession_no,
        url=url,
        form=kv.str("form", "10-K"),
...
def register_filings_commands() -> None:
    ...
    register_command(FinanceCommand(
        ...
        _filings_read,
        usage="filings.read [SYMBOL] [ACCESSION|accession=ACCESSION|url=URL] [form=10-K section=business|risk_factors|mda|segments max_chars=8000]",
        examples=(
